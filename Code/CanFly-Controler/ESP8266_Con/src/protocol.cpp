...
{
    int len = queue_insert(canfly_ctr.rxtemp,canfly_ctr.rxlen);
    while(queue_size()>ADDI_BYTE_NUM){
        ...
        for(int i=0;i<data_len+ADDI_BYTE_NUM;i++){
            canfly_ctr.wtxtemp[i+canfly_ctr.wtxlen]=queue_peek(1);
            queue_pop();
        }
        if(ano_data_check(canfly_ctr.wtxtemp+canfly_ctr.wtxlen,data_len+ADDI_BYTE_NUM)==true){
            canfly_ctr.wtxlen+=data_len+ADDI_BYTE_NUM;
        }
    }
    if(canfly_ctr.wtxlen==0)return false;
    else return true;
}