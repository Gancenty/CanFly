	  ...
	  *len=0;
		...
		*len=total;
		switch(data[2]){
			...
			case PWM_INFO:
				next=ano_data_get_u16bit(data,4,&ret2);
				canfly_ctr.pwm1=ret2;
				next=ano_data_get_u16bit(data,next,&ret2);
				canfly_ctr.pwm2=ret2;
				next=ano_data_get_u16bit(data,next,&ret2);
				canfly_ctr.pwm3=ret2;
				next=ano_data_get_u16bit(data,next,&ret2);
				canfly_ctr.pwm4=ret2;
				break;
			case INERTIAL_SENSOR:
				next=ano_data_get_s16bit(data,4,&ret1);
				canfly_ctr.acc_x=ret1;
				next=ano_data_get_s16bit(data,next,&ret1);
				canfly_ctr.acc_y=ret1;
				next=ano_data_get_s16bit(data,next,&ret1);
				canfly_ctr.acc_z=ret1;
				next=ano_data_get_s16bit(data,next,&ret1);
				canfly_ctr.gyr_x=ret1;
				next=ano_data_get_s16bit(data,next,&ret1);
				canfly_ctr.gyr_y=ret1;
				next=ano_data_get_s16bit(data,next,&ret1);
				canfly_ctr.gyr_z=ret1;
				break;
			default:break;
		}
