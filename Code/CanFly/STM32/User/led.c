...
volatile uint16_t LED_Buffer[DATA_SIZE*LED_NUM+1];
...
uint16_t WS2812B_Write_24Bits(uint16_t num,uint32_t GRB_Data)
{
  uint16_t i=0;
	...
	for(i=0;i<3;i++){
		color[i]=(float)color[i]*((float)canfly.bright/255);
	}
...
void WS2812B_Blink(void)//GRB
{
	uint32_t stay_color[][4]={0xff0000,0x00ff00,0xff00ee,0xff0aa0,\
											 0xff0aa0,0xff0000,0x00ff00,0xff00ee,\
											 0xff00ee,0xff0aa0,0xff0000,0x00ff00,\
											 0x00ff00,0xff00ee,0xff0aa0,0xff0000,\
											};
	uint32_t unlock_color[][4]={0x4a7af9,0x00ff00,0x00ff00,0x4a7af9};
	static uint8_t flag=0;
	if(canfly.tim_ms-canfly.last_rec>200){
		canfly.status&=~CONNECT_MARK;
	}else{
		canfly.status|=CONNECTED;
	}
	static uint8_t change=0;
	switch(canfly.status&POWER_MARK){
		case FULL_POWER:		
			switch(canfly.status&CONNECT_MARK){			
				case CONNECTED:				
					switch(canfly.status&FLY_MARK){
						case IN_FLY_MODE:
							WS2812B_Color(unlock_color[0],LED_NUM);
							break;
						case STAY_MODE:
							WS2812B_Color(stay_color[change],LED_NUM);
							change=(change+1)%LED_NUM;
							break;
						default:break;
					}
					break;				
